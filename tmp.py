...
class Teeth3DSDataset(Dataset):
    ...
    def __init__(self, root: str, processed_folder: str = 'processed',
                 in_memory: bool = False, pre_transform=None, post_transform=None,
        ...
        Path(join(self.root, self.processed_folder, 'upper')).mkdir(parents=True, exist_ok=True)
        Path(join(self.root, self.processed_folder, 'lower')).mkdir(parents=True, exist_ok=True)
        ...
        for f in split_files:
            with open(f'.datasets/teeth3ds/Teeth3DS_split/{f}') as file:
                for l in file:
                    ...
                    if 'lower' in l:
                        self.file_names.append(join(self.root, self.processed_folder, 'lower', l))
                    elif 'upper' in l:
                        self.file_names.append(join(self.root, self.processed_folder, 'upper', l))
    ...
    def _is_processed(self):
        ...
        for view in self.mesh_view:
            raw_mesh_folder = join(self.root, view)
            process_mesh_folder = join(self.root, self.processed_folder, view)
    ...
    def __getitem__(self, index):
        if self.in_memory:
            ...
            return pointcloud.to(torch.float), labels  # point_coords, face_info
